...
const Cart = () => {
    ...
    return (
        <div className="container mx-auto py-8 px-4 sm:py-12 sm:px-6">
            ...
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-serif mb-6 sm:mb-12 text-center md:text-left">Shopping Cart</h1>
            ...
            {cartItems.length > 0 ? (
                <div className="flex flex-col lg:flex-row gap-8 sm:gap-12">
                            ...
                            {cartItems.map(item => (
                                <div key={item.id} className="flex gap-3 sm:gap-6 p-3 sm:p-6 border-b border-gray-100 last:border-0 items-center">
                                    ...
                                    <div className="flex-grow min-w-0">
                                        ...
                                        <h3 className="font-serif text-sm sm:text-base md:text-lg font-medium truncate">{item.product?.title || 'Unavailable'}</h3>
                    ...
                    <div className="w-full lg:w-1/3">
                        <div className="bg-white p-6 sm:p-8 shadow-sm border border-gray-100 lg:sticky lg:top-24">
                            <h3 className="text-lg sm:text-xl lg:text-2xl font-serif mb-4 sm:mb-6">Order Summary</h3>
                ...
                <div className="text-center py-12 sm:py-16">
                    ...
                    <p className="text-sm sm:text-base text-gray-400 mb-6 sm:mb-8">Add items to your cart to proceed.</p>
                    <Link
                        to="/category"
                                ...
                                {featuredProducts.map((product) => (
                                    ...
                                    >
                                        ...
                                        <div className="p-3 sm:p-5 text-left flex flex-col flex-grow">
                                            <p className="text-[10px] sm:text-xs text-heritage-bronze/80 uppercase tracking-widest truncate">{product.category}</p>
                                            {product.seller?.name && (
                                                <p className="text-[10px] sm:text-xs text-heritage-charcoal/50 truncate">by {product.seller.name}</p>
                                            )}
                                            <h3 className="font-serif text-sm sm:text-base md:text-lg text-heritage-charcoal leading-tight sm:leading-snug line-clamp-2 mt-0.5 sm:mt-1">{product.title}</h3>
                                            <p className="text-heritage-gold-muted font-sans text-xs sm:text-base lg:text-lg font-semibold mt-1 sm:mt-1.5">₹{product.price?.toLocaleString()}</p>
                                        </div>
                                    </Link>
                                ))}
                            </div>

...
import { ArrowLeft, Clock, User, BookOpen, Share2, ArrowRight, Check, List, BookMarked, Quote } from 'lucide-react';
...
function BlogPost() {
    ...
    const { related } = (() => {
        ...
        return { related: (relatedData?.posts || []).filter(p => p.slug !== slug).slice(0, 3) };
    })();
        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <Link to="/archive" className="inline-flex items-center gap-2 text-sm uppercase tracking-widest text-luxury-gold hover:text-heritage-brown transition-colors">
    ...
    return (
        <div className="min-h-screen bg-white">
            ...
            <div className="fixed top-0 left-0 right-0 z-50 h-0.5 bg-gray-100">
                <div className="h-full bg-luxury-gold transition-all duration-150 ease-out" style={{ width: `${progress}%` }} />
            </div>
            ...
            {post.coverImage ? (
                <section className="relative h-[40vh] sm:h-[50vh] lg:h-[60vh] min-h-[300px] overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-12 lg:p-16">
                        <div className="container mx-auto max-w-4xl">
                            <Link to="/archive" className="inline-flex items-center gap-2 text-white/60 hover:text-luxury-gold text-xs uppercase tracking-widest transition-colors mb-4 sm:mb-6">
                                <ArrowLeft size={14} /> Back to Archive
                            </Link>
                            <div className="flex items-center flex-wrap gap-2 mb-3">
                                <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-luxury-gold bg-black/30 px-3 py-1.5 rounded-full backdrop-blur-sm">{post.category}</span>
                                {post.readingTime && (
                            ...
                            <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-serif text-white leading-tight max-w-3xl">{post.title}</h1>
                        </div>
                    </div>
                </section>
            ) : (
                <section className="relative py-20 sm:py-28 px-6 bg-heritage-charcoal overflow-hidden">
                    ...
                    <div className="container mx-auto max-w-4xl relative z-10">
                        <Link to="/archive" className="inline-flex items-center gap-2 text-white/60 hover:text-luxury-gold text-xs uppercase tracking-widest transition-colors mb-8">
                            <ArrowLeft size={14} /> Back to Archive
                        </Link>
                        <div className="flex items-center flex-wrap gap-2 mb-4">
                            <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-luxury-gold bg-black/20 px-3 py-1.5 rounded-full">{post.category}</span>
                            {post.readingTime && (
                        ...
                        <h1 className="text-2xl sm:text-4xl md:text-5xl font-serif text-white leading-tight max-w-3xl">{post.title}</h1>
                    </div>
                </section>
            )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs sm:text-sm text-gray-500">
                            ...
                            <span className="flex items-center gap-1.5">
                                <Clock size={14} />
                                {post.publishedAt ? new Date(post.publishedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : new Date(post.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                            </span>
                            {post.tags?.length > 0 && (
                                <>
                                    <span className="text-gray-300">|</span>
                                    <span className="flex items-center flex-wrap gap-1.5">
                                        {post.tags.map(tag => (
                                            <span key={tag} className="text-[10px] bg-gray-50 px-2 py-0.5 rounded-full">#{tag}</span>
                                        ))}
                                    </span>
                                </>
                            )}
                        ...
                        <aside className="hidden xl:block w-56 flex-shrink-0 py-12">
                            <div className="sticky top-24">
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <span className="text-xs uppercase tracking-widest font-bold text-gray-400">Contents</span>
                                </div>
                    ...
                    <main className="flex-1 min-w-0 py-10 sm:py-16">
                        ...
                        <div className="mt-12 sm:mt-16 pt-8 border-t border-gray-100">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <p className="font-serif font-bold text-heritage-charcoal">Written by {post.author}</p>
                                    <p className="text-sm text-gray-500 mt-1 leading-relaxed">
                                        The Collectors Exchange is a curated marketplace for verified pre-owned collectibles and antiques. 
                                        Our editorial team brings you stories from the world of horology, gemology, art, and collecting.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <section className="py-16 sm:py-20 px-6 bg-heritage-cream/50 mt-4">
                    <div className="container mx-auto max-w-6xl">
                        <div className="flex items-center gap-3 mb-10">
                            ...
                            <h2 className="text-sm uppercase tracking-[0.2em] font-bold text-heritage-charcoal">Related Articles</h2>
                            <div className="h-px flex-1 bg-gray-200" />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                            {related.map(r => (
                                <Link key={r.id} to={`/archive/${r.slug}`}
                                    ...
                                    <div className="p-5">
                                        <span className="text-[9px] uppercase tracking-[0.2em] font-bold text-luxury-gold bg-luxury-gold/5 px-2 py-0.5 rounded-full">{r.category}</span>
                                        <h3 className="font-serif font-bold text-heritage-charcoal group-hover:text-luxury-gold transition-colors mt-2 mb-2 line-clamp-2">{r.title}</h3>
            ...
            <section className="py-16 px-6 text-center bg-white">
                <div className="container mx-auto max-w-lg">
                    ...
                    <p className="text-gray-500 text-sm mb-6">Browse the full archive for more insights into the world of collecting.</p>
                    <Link to="/archive"

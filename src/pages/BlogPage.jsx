    ...
    <Link to={`/archive/${post.slug}`}
        ...
        <div className="p-4 sm:p-6 flex flex-col flex-1">
            <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold bg-luxury-gold/5 px-2.5 py-1 rounded-sm">{post.category}</span>
                {post.featured && (
                    <span className="text-[10px] uppercase tracking-widest font-bold text-heritage-charcoal bg-heritage-charcoal/5 px-2.5 py-1 rounded-sm">Featured</span>
                )}
            </div>
            <h3 className="font-serif text-base sm:text-lg font-bold text-heritage-charcoal group-hover:text-luxury-gold transition-colors duration-300 mb-2 line-clamp-2">{post.title}</h3>
            ...
            <section className="sticky top-0 bg-white/95 backdrop-blur border-b border-gray-100 z-30">
                <div className="container mx-auto px-4 sm:px-6 py-3 sm:py-4">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div className="overflow-x-auto scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0 flex-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                ...
                <div className="container mx-auto max-w-6xl">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-20 sm:py-32">
                            ...
                            <p className="text-gray-400 font-serif italic text-sm sm:text-base">Opening the archive...</p>
                        </div>
                    ) : posts.length === 0 ? (
                        <div className="text-center py-16 sm:py-24">
                            <BookOpen size={40} className="mx-auto text-gray-200 mb-4 sm:w-14 sm:h-14" strokeWidth={1} />
                                        ...
                                        <Link key={post.id} to={`/archive/${post.slug}`}
                                            ...
                                            <div className="grid md:grid-cols-2 min-h-[260px] sm:min-h-[300px]">
                                                ...
                                                <div className="p-6 sm:p-12 flex flex-col justify-center">
                                                    <span className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold mb-3">{post.category}</span>
                                                    <h3 className="text-xl sm:text-3xl font-serif text-white leading-tight mb-4 group-hover:text-luxury-gold transition-colors">{post.title}</h3>

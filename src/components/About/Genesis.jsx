import React from 'react';
import { Reveal, Stagger } from '../Motion';

const Genesis = () => (
  <section className="bg-cream text-obsidian py-16 sm:py-24 px-6 md:px-12 lg:px-24 snap-start">
    <div className="max-w-3xl mx-auto">
      <Reveal
        as="div"
        className="text-2xl sm:text-3xl md:text-4xl font-serif uppercase tracking-widest text-brass mb-3 sm:mb-4"
      >
        03 / THE GENESIS
      </Reveal>
      <Reveal
        as="h2"
        blur
        delay={100}
        className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold mb-4 sm:mb-6"
      >
        From Restless Curiosity to a Curated Doorstep Reality.
      </Reveal>
      <Stagger>
        <p className="text-sm sm:text-base md:text-lg font-light leading-relaxed mb-4">
          The Collectors Exchange was born from a deep-rooted passion for the past and a restless
          curiosity about the unspoken stories objects hold. We believe that every item we
          rediscover secures its rightful place in history once again.
        </p>
        <p className="text-sm sm:text-base md:text-lg font-light leading-relaxed">
          For many, owning a century-old timepiece or a rare philatelic treasure has long felt like
          an unreachable dream, a complex nightmare of opaque pricing, counterfeits, and
          geographical friction. We founded this platform to end that struggle. We navigate the
          intricate labyrinth of vintage markets so you don't have to, delivering authenticated
          history directly to your doorstep.
        </p>
      </Stagger>
    </div>
  </section>
);

export default Genesis;

...
const Header = () => {
    ...
    return (
        <>
                    ...
                    <div className="flex flex-col h-full pt-12">
                        ...
                        <nav className="flex-1 overflow-y-auto py-4 px-5">
                                ...
                                {navItems.map((item, index) => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={`group relative text-sm font-medium uppercase tracking-[0.2em] py-3.5 px-4 rounded-sm transition-all duration-400 ease-out ${isActive
                                        ...
                                        >
                                            ...
                                            <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-0 transition-all duration-300 rounded-full ${isActive ? 'bg-luxury-gold h-3/5' : 'group-hover:bg-luxury-gold/50 group-hover:h-2/5'}`} />
                                        </Link>
                                    );
                                })}
                            </div>
                            ...
                            <Link
                                to="/links"
                                onClick={() => setIsMenuOpen(false)}
                                className={`group relative flex items-center text-sm font-medium uppercase tracking-[0.2em] py-3.5 px-4 rounded-sm transition-all duration-400 ease-out ${location.pathname === '/links'
                                    ? 'text-luxury-gold bg-luxury-gold/5'
                                    : 'text-heritage-charcoal hover:text-luxury-gold hover:bg-heritage-cream'
                                    } ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}
                            >
                                <span className="relative z-10">Links</span>
                                <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-0 transition-all duration-300 rounded-full ${location.pathname === '/links' ? 'bg-luxury-gold h-3/5' : 'group-hover:bg-luxury-gold/50 group-hover:h-2/5'}`} />
                            </Link>
                        </nav>
                        {/* Drawer footer */}
            ...
            <nav aria-label="Mobile navigation" className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur border-t border-gray-200 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] flex items-center justify-around pt-1.5 pb-[calc(0.5rem+env(safe-area-inset-bottom))] transition-transform duration-500 ease-out ${showNav ? 'translate-y-0' : 'translate-y-full'}`}>
                {bottomNav.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`absolute -top-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full transition-all duration-300 ${isActive ? 'bg-luxury-gold opacity-100 scale-100' : 'opacity-0 scale-0'}`} />
                            <div className="relative transition-transform duration-300 ease-out group-hover:scale-110">
                                ...
                                />
                                {item.count > 0 && (
                                    <span className={`absolute -top-1.5 -right-2 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold transition-all duration-300 ${isActive ? 'bg-luxury-gold scale-110' : 'bg-luxury-gold'}`}>
                                        {item.count > 9 ? '9+' : item.count}
                                    </span>
                                )}
                            </div>
                            <span className={`text-[10px] uppercase tracking-wider font-medium leading-none transition-all duration-300 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-60 group-hover:opacity-100'}`}>
                                {item.name}
                            </span>
                        </Link>
                    );

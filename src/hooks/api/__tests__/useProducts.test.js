...
describe('useProducts', () => {
  ...
  describe('useUnpublishProduct', () => {
    it('sends patch request with remark to unpublish', async () => {
      mockApiClient.patch.mockResolvedValue({
        data: { id: '1', isPublished: false, unpublishRemark: 'Sold locally' },
      });
      const { useUnpublishProduct } = await import('../useProducts');
      const { result } = renderHook(() => useUnpublishProduct(), { wrapper: createWrapper() });
      result.current.mutate({ id: '1', remark: 'Sold locally' });
      await waitFor(() => expect(result.current.isSuccess).toBe(true));
      expect(mockApiClient.patch).toHaveBeenCalledWith('/products/1/unpublish', {
        remark: 'Sold locally',
      });
    });

    it('handles API error on unpublish', async () => {
      mockApiClient.patch.mockRejectedValue(new Error('Unpublish failed'));
      const { useUnpublishProduct } = await import('../useProducts');
      const { result } = renderHook(() => useUnpublishProduct(), { wrapper: createWrapper() });
      result.current.mutate({ id: '999', remark: 'Sold locally' });
      await waitFor(() => expect(result.current.isError).toBe(true));
    });
  });
});

  ...
  describe('useProducts', () => {
    ...
    it('includes listingCategory parameter when provided', async () => {
      ...
      expect(mockApiClient.get).toHaveBeenCalledWith(
        '/products?listingCategory=premium&page=1&limit=12',
      );
    });
    ...
    it('includes condition parameter when provided', async () => {
      mockApiClient.get.mockResolvedValue({ data: [] });
      const { useProducts } = await import('../useProducts');
      renderHook(() => useProducts(null, null, 1, 12, null, 'Excellent'), {
        wrapper: createWrapper(),
      });
      await waitFor(() => expect(mockApiClient.get).toHaveBeenCalled());
      expect(mockApiClient.get).toHaveBeenCalledWith(
        '/products?condition=Excellent&page=1&limit=12',
      );
    });
    ...
    it('includes sort when provided', async () => {
      mockApiClient.get.mockResolvedValue({ data: [] });
      const { useProducts } = await import('../useProducts');
      renderHook(() => useProducts(null, null, 1, 12, null, null, { sort: 'price_asc' }), {
        wrapper: createWrapper(),
      });
      await waitFor(() => expect(mockApiClient.get).toHaveBeenCalled());
      expect(mockApiClient.get).toHaveBeenCalledWith('/products?sort=price_asc&page=1&limit=12');
    });
    ...
    it('includes price bounds when provided', async () => {
      ...
      const { useProducts } = await import('../useProducts');
      renderHook(
        () => useProducts(null, null, 1, 12, null, null, { minPrice: 2000, maxPrice: 500000 }),
        { wrapper: createWrapper() },
      );
      await waitFor(() => expect(mockApiClient.get).toHaveBeenCalled());
      expect(mockApiClient.get).toHaveBeenCalledWith(
        '/products?minPrice=2000&maxPrice=500000&page=1&limit=12',
      );
    });

    it('omits blank and undefined price bounds', async () => {
      mockApiClient.get.mockResolvedValue({ data: [] });
      const { useProducts } = await import('../useProducts');
      renderHook(
        () => useProducts(null, null, 1, 12, null, null, { minPrice: '', maxPrice: undefined }),
        { wrapper: createWrapper() },
      );
      await waitFor(() => expect(mockApiClient.get).toHaveBeenCalled());
      expect(mockApiClient.get).toHaveBeenCalledWith('/products?page=1&limit=12');
    ...
    it('sends a zero lower bound rather than dropping it as falsy', async () => {
      mockApiClient.get.mockResolvedValue({ data: [] });
      const { useProducts } = await import('../useProducts');
      renderHook(() => useProducts(null, null, 1, 12, null, null, { minPrice: 0 }), {
        wrapper: createWrapper(),
      });
      await waitFor(() => expect(mockApiClient.get).toHaveBeenCalled());
      expect(mockApiClient.get).toHaveBeenCalledWith('/products?minPrice=0&page=1&limit=12');
    });

    it('keeps a separate cache entry per sort so results are not cross-served', async () => {
      mockApiClient.get.mockResolvedValue({ data: [] });
      const { useProducts } = await import('../useProducts');
      ...
      renderHook(() => useProducts(null, null, 1, 12, null, null, { sort: 'price_desc' }), {
        wrapper,
      });

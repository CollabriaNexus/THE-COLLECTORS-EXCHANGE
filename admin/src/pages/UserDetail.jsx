...
function UserDetail() {
  ...
  const handleToggleVendorType = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to update vendor type'));
    }
  };

  const handleBan = async () => {
    // Banning locks the account out and fires a notification to the user.
    // It used to happen on a single unguarded click.
    const confirmed = await confirm(
      `Ban ${user.name || user.email}? They will be locked out of their account immediately and told their account has been banned. You can unban them again from this page.`,
    );
    if (!confirmed) return;
    setError('');
    try {
  ...
  // A 500 is not a missing user — don't report a load failure as "not found".
  if (isError) {
    return (
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-serif font-bold text-heritage-charcoal">User Details</h2>
        ...
        <div className="bg-white rounded-lg shadow-heritage p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <dd className="mt-1">
                <span
                  className={`px-2 py-1 rounded-full text-xs font-semibold ${user.banned ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}
                >
          ...
          <div className="mt-6 pt-6 border-t border-gray-200 space-y-4">
            ...
            <div className="pt-4 border-t border-gray-200">
              {user.banned ? (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-green-50 text-green-700 py-2 rounded-md font-medium hover:bg-green-100 transition-colors disabled:opacity-50"
                >
                  <CheckCircle size={18} />
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-red-50 text-red-700 py-2 rounded-md font-medium hover:bg-red-100 transition-colors disabled:opacity-50"
                >
                  <Ban size={18} />
            ...
            {user.kycStatus === 'verified' && (
              <div className="pt-4 border-t border-gray-100">
                <h4 className="text-sm font-semibold text-heritage-charcoal mb-2 font-serif">
                  Vendor Type
                </h4>
                <div className="bg-gray-50 p-3 rounded border border-gray-100 mb-3 text-xs text-gray-600 space-y-1">
                  <p>
                    <strong>Status:</strong>{' '}
                    <span className="capitalize">{user.vendor?.status || 'None'}</span>
                  </p>
                  <p>
                    <strong>Type:</strong>{' '}
                    <span
                      className={`px-2 py-0.5 rounded font-medium ${user.vendor?.type === 'BULK' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'}`}
                    >
                      {user.vendor?.type === 'BULK' ? 'Bulk Lister' : 'Normal'}
                    </span>
                  </p>
                  <p>
                    <strong>Max Listings:</strong> {user.vendor?.maxListings ?? 5}
                  </p>
                </div>
                <button
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg shadow-heritage p-6">
              ...
              <p className="text-gray-500 text-sm">{user.cart?.length || 0} items in cart</p>
            </div>

...
function Testimonials() {
  ...
  const onMutationError = (fallback) => (err) => setActionError(getErrorMessage(err, fallback));
  ...
  const approveMutation = useMutation({
    ...
    onError: onMutationError('Could not approve that testimonial.'),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: onMutationError('Could not reject that testimonial.'),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: onMutationError('Could not delete that testimonial.'),
  });
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <span
                        key={i}
                        className={`text-sm ${i <= t.rating ? 'text-amber-400' : 'text-gray-200'}`}
                      >
                        ★
                      </span>
                    ))}
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Status:{' '}
                  <span
                    className={`font-medium ${t.status === 'APPROVED' ? 'text-green-600' : t.status === 'REJECTED' ? 'text-red-600' : 'text-amber-600'}`}
                  >
                    {t.status}
                  </span>
                </p>
              ...
              <div className="flex items-center gap-2 ml-4">
                ...
                <button
                  ...
                  onClick={async () => {
                    if (
                      await confirm(
                        `Delete the testimonial from ${t.authorName}? This removes it permanently and cannot be undone.`,
                      )
                    )

...
const TCEStore = () => {
  const { data, isLoading, isError, error, refetch, isFetching } = useTCEProducts();
  ...
  const handleFileUpload = async (index, file) => {
    ...
    try {
      ...
      setFormError(
        getErrorMessage(err, 'Failed to upload the image. Try again, or paste a URL directly.'),
      );
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validImages.length < 1) {
      setFormError('Please provide at least one image.');
      return;
    }
    const payload = {
      title: form.title,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Products listed by The Collectors Exchange — auto-verified, auto-published
          </p>
        </div>
        <button
          onClick={() => (showForm ? cancelForm() : setShowForm(true))}
          className="flex items-center gap-2 px-4 py-2 bg-luxury-gold text-black rounded-md hover:bg-luxury-gold/90 transition-colors font-medium text-sm"
        >
        ...
        <div className="bg-white border border-gray-100 shadow-sm mb-6">
          <div className="p-10">
            <div className="mb-8 pb-8 border-b border-gray-100">
              <h3 className="text-3xl font-serif mb-2 text-heritage-charcoal">
                {editingId ? 'Edit TCE Listing' : 'New TCE Listing'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-gray-50 p-6 border border-gray-100 rounded-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-gray-500">
                    {form.imageUrls.filter((u) => u).length} / 10 Images
                  </span>
                </div>
                ...
                <div className="mt-4 flex items-start gap-2 text-xs text-gray-400 bg-white p-3 border border-gray-100">
                  ...
                  <p>
                    Upload images directly or paste image URLs. The first image will be the primary
                    detailed view and card thumbnail.{' '}
                    {imageUploading && (
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end">
                <button
                  ...
                  className="bg-heritage-charcoal text-white px-12 py-4 text-sm uppercase tracking-widest hover:bg-heritage-dark transition-colors shadow-lg flex items-center justify-center gap-2"
                >
      ...
      {isLoading ? (
        ...
        <ErrorState
          error={error}
          title="Could not load TCE Store products"
          onRetry={refetch}
            ...
            >
              <Link to={`/products/${product.id}`}>
                <div className="relative aspect-square bg-gray-50 overflow-hidden">
                  ...
                  {/* The badge used to be hard-coded to "Verified" for
                                        every card, so a Sold or Rejected TCE listing still
                                        advertised itself as live and verified. */}
                  {product.status === 'Approved' && product.isPublished ? (
                    ...
                    <div className="absolute top-2 left-2 bg-gray-700 text-white text-[10px] px-2 py-0.5 rounded flex items-center gap-1">
                      {product.status === 'Sold' ? 'Sold' : product.status || 'Hidden'}
                    </div>
                  )}
                </div>
                <div className="p-3">
                  ...
                  <h3 className="font-serif text-sm font-medium text-heritage-charcoal line-clamp-1 mt-0.5">
                    {product.title}
                  </h3>
              ...
              <button
                ...
                className="absolute top-2 right-2 p-1.5 bg-white/90 rounded text-gray-400 hover:text-luxury-gold transition-colors shadow-sm opacity-0 group-hover:opacity-100"
                title="Edit product"
              >
                <Edit3 size={14} />
              </button>

...
function Vendors() {
  ...
  const handleToggleType = async (userId, currentType) => {
    ...
    try {
      ...
      setSuccessMsg(
        `Vendor type changed to ${newType === 'BULK' ? 'Bulk Lister (unlimited listings)' : 'Normal (5 listings)'}`,
      );
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err) {
      // Was console.error only: a failed toggle looked exactly like a
      // successful one because the button state never changed either.
      setSuccessMsg('');
      setErrorMsg(getErrorMessage(err, 'Failed to change the vendor type.'));
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded text-xs font-medium ${type === 'BULK' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'}`}
          >
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-3xl font-serif font-bold text-heritage-charcoal">Vendor Management</h2>
      ...
      {successMsg && (
        <div
          role="status"
          className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded mb-6 flex items-center gap-2"
        >
          <ShieldCheck size={16} />
          {successMsg}
        </div>

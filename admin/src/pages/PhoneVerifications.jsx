...
function PhoneVerifications() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => {
      setActionError('');
      queryClient.invalidateQueries({ queryKey: ['admin', 'phone-verifications'] });
    },
    onError: (err) => setActionError(getErrorMessage(err, 'Could not verify that phone number.')),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setActionError(getErrorMessage(err, 'Could not reject that phone number.')),
  });

  const handleReject = async (user) => {
    const confirmed = await confirm(
      `Reject the phone number ${user.phone || '(none)'} for ${user.name || user.email}? They will need to submit a new number.`,
    );
    if (confirmed) rejectMutation.mutate(user.id);
  };

  // Only the row being acted on should show a busy state; keying off the bare
  // isPending flag greyed out every other row's buttons too.
  const busyId =
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <ErrorState
          error={error}
          title="Could not load phone verifications"
          onRetry={refetch}
        ...
        <div className="space-y-4">
          {users?.map((u) => (
            <div
              key={u.id}
              className="bg-white border border-gray-100 p-6 flex items-start justify-between"
            >
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <span className="font-serif font-bold text-heritage-charcoal">
                    {u.name || 'Unknown'}
                  </span>
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Status:{' '}
                  <span
                    className={`font-medium ${u.phoneVerificationStatus === 'verified' ? 'text-green-600' : u.phoneVerificationStatus === 'rejected' ? 'text-red-600' : 'text-amber-600'}`}
                  >
                    {u.phoneVerificationStatus}
                  </span>
              ...
              {u.phoneVerificationStatus === 'pending' && (
                <div className="flex items-center gap-2 ml-4">
                  <button
                    ...
                    className="p-2 bg-green-50 text-green-600 rounded hover:bg-green-100 disabled:opacity-40"
                    title="Approve"
                  >
                    <CheckCircle size={18} />
                  </button>
                  <button
                    ...
                    disabled={busyId === u.id}
                    className="p-2 bg-red-50 text-red-600 rounded hover:bg-red-100 disabled:opacity-40"
                    title="Reject"
                  >
                    <XCircle size={18} />
                  </button>

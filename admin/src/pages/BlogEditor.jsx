...
function BlogEditor() {
  ...
  const handleCoverUpload = async (e) => {
    ...
    try {
      ...
      setSaveError(getErrorMessage(err, 'The cover image failed to upload.'));
    } finally {
  ...
  const insertImage = useCallback(async () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        setSaveError(getErrorMessage(err, 'That image failed to upload.'));
      } finally {
  ...
  const handleSubmit = async (publishStatus) => {
    ...
    try {
      ...
      // Previously console.error only: a rejected save (duplicate slug,
      // validation failure, 403) left the editor sitting there looking
      // idle, and the writer had no idea the article was never stored.
      setSaveError(
        getErrorMessage(err, 'The post could not be saved. Your text is still here — try again.'),
      );
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {isEditing ? 'Update your article.' : 'Write a new archival article.'}
          </p>
        </div>
      ...
      {saveError && (
        <div
          role="alert"
          className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-5 text-sm"
        >
          {saveError}
        </div>
      )}
      ...
      <div className="space-y-5">
          ...
          <div className="flex items-end pb-2.5">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm font-medium text-gray-700">
                Featured post (shows as spotlight)
              </span>
            </label>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="flex items-center gap-0.5 px-3 py-2 border-b border-gray-100 bg-gray-50/50 flex-wrap">
            ...
            {contentImageUploading && (
              <Loader2 size={14} className="animate-spin text-luxury-gold ml-2" />
            )}
          </div>
            ...
            <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm mx-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  type="button"
                  onClick={addLink}
                  className="px-4 py-2 text-sm bg-luxury-gold text-white rounded-lg hover:bg-heritage-brown"
                >
                  Apply
                </button>
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          >
            ...
            <ChevronDown
              size={16}
              className={`text-gray-400 transition-transform ${showSeo ? 'rotate-180' : ''}`}
            />
          </button>
          {showSeo && (

...
describe('PhoneVerifications', () => {
  ...
  it('renders verification requests', async () => {
    mockGet.mockResolvedValue({
      data: [{ id: 'u1', name: 'Alice', email: 'alice@test.com', phone: '9999999999', phoneVerificationStatus: 'pending', createdAt: '2024-01-01T00:00:00Z' }],
    });
    render(<PhoneVerifications />, { wrapper: createWrapper() });
    await waitFor(() => {
  ...
  it('approves a verification', async () => {
    ...
    mockGet.mockResolvedValue({
      data: [{ id: 'u1', name: 'Bob', email: 'bob@test.com', phone: '8888888888', phoneVerificationStatus: 'pending', createdAt: '2024-01-01T00:00:00Z' }],
    });
    render(<PhoneVerifications />, { wrapper: createWrapper() });
    await waitFor(() => {
  ...
  it('does not show action buttons for non-pending status', async () => {
    mockGet.mockResolvedValue({
      data: [{ id: 'u1', name: 'Alice', email: 'alice@test.com', phone: '9999999999', phoneVerificationStatus: 'verified', createdAt: '2024-01-01T00:00:00Z' }],
    });
    render(<PhoneVerifications />, { wrapper: createWrapper() });
    await waitFor(() => {
